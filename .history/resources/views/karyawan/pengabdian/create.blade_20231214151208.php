                    ...
                    <form method="post" action="{{ route ('karyawan.pengabdian.store')}}" enctype="multipart/form-data">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="col-12">
                                    <label class="form-label">Personil Yang Terlibat</label>
                                    <select name="pkm_personilterlibat[]" class="form-control" multiple>
                                        @foreach ($penggunas as $pengguna)
                                        <option value="{{ $pengguna->pgn_id ?? '' }}" {{ is_array(old('pgn_id')) && in_array($pengguna->pgn_id, old('pgn_id', [])) ? 'selected' : '' }}>
                                            {{ $pengguna->pgn_nama ?? '' }}
                                        </option>
                                        @endforeach
                                    </select>
