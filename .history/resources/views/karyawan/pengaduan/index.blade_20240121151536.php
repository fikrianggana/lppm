...
<script>
    $(document).ready(function () {
        $("#datajenispengaduan").change(function () {
            ...
            var selectedId = anggotaId; // Gantilah ini dengan logika pengambilan ID yang sesungguhnya

            // Lakukan AJAX request ke metode controller Laravel
            $.ajax({
                url: `{{ route('pengaduan.getData', ['id' => selectedId, 'pengaduan' => selectedPengaduan]) }}`,

                type: "GET",
                success: function (data) {
                    // Kosongkan pilihan pada select "databro" sebelum diisi kembali
                    $("#databro").empty();

                    // Isi kembali select "databro" dengan data yang diperoleh dari controller
